...
/*   Updated: 2024/07/15 19:36:42 by lgasc            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_putnbr_fd(const int integer, const int file_descriptor)
{
	...
	length = 1;
	while (integer / (signed size_t){power(10, length - 1)} <= -10
		|| integer / (signed size_t){power(10, length - 1)} >= 10)
		++ length;
	i = 0;
	while (i < length)
	{
		write(file_descriptor, "0123456789" + ((integer / (signed size_t)
